...
DFRobot_NeoPixel RGB_LED(1, 12, NEO_GRB + NEO_KHZ800);
/*!
  ...
  @param   t  Pixel type -- add together NEO_* constants defined in
              DFRobot_NeoPixel.h, for example NEO_GRB+NEO_KHZ800 for
              NeoPixels expecting an 800 KHz (vs 400 KHz) data stream
              with color bytes expressed in green, red, blue order per
              pixel.
  @return  DFRobot_NeoPixel object. Call the begin() function before use.
*/

DFRobot_NeoPixel::DFRobot_NeoPixel(uint16_t n, uint8_t p, neoPixelType t) :
...
/*!
  @brief   Deallocate DFRobot_NeoPixel object, set data pin back to INPUT.
*/
DFRobot_NeoPixel::~DFRobot_NeoPixel() {
  free(pixels);
...
*/
void DFRobot_NeoPixel::begin(void) {
  if(pin >= 0) {
    pinMode(pin, OUTPUT);
...
/*!
  @brief   Change the length of a previously-declared DFRobot_NeoPixel
...
/*!
  @brief   Change the pixel format of a previously-declared
           DFRobot_NeoPixel strip object. If format changes from one of
...
*/
void DFRobot_NeoPixel::setPixelColor(uint16_t n, uint32_t c) {
  if(n < numLEDs) {
    uint8_t *p,
      r = (uint8_t)(c >> 16),
      g = (uint8_t)(c >>  8),
...
*/
void DFRobot_NeoPixel::fill(uint32_t c, uint16_t first, uint16_t count) {
  uint16_t i, end;
...
*/
uint32_t DFRobot_NeoPixel::ColorHSV(uint16_t hue, uint8_t sat, uint8_t val) {
  ...
  return (((((((r/2)* s1) >> 8) + s2) * v1) & 0xff00) << 8) |
          (((((g * s1) >> 8) + s2) * v1) & 0xff00)       |
         ( ((((b * s1) >> 8) + s2) * v1)           >> 8);
}
...
*/
void DFRobot_NeoPixel::clear(void) {
  memset(pixels, 0, numBytes);
}

// A 32-bit variant of gamma8() that applies the same function
// to all components of a packed RGB or WRGB value.
uint32_t DFRobot_NeoPixel::gamma32(uint32_t x) {
  uint8_t *y = (uint8_t *)&x;
...
void DFRobot_NeoPixel::setBrightness(uint8_t brightness){
    this->_brightness = brightness;
}

void DFRobot_NeoPixel::setColor(uint8_t color){
    switch(color){
        case RED:
            this->_r = this->_brightness;
            this->_g = 0;
            this->_b = 0;
        ...
        case GREEN:
            this->_r = 0;
            this->_g = this->_brightness;
            this->_b = 0;
            break;
        case BLUE:
            this->_r = 0;
            this->_g = 0;
            this->_b = this->_brightness;
            break;
        case YELLOW:
            this->_r = (this->_brightness)/2;
            this->_g = this->_brightness;
            this->_b = 0;
            break;
        case PURPLE:
    ...
    this->nowColor = Color(this->_r,this->_g,this->_b);
    setPixelColor(0,this->nowColor);
}
void DFRobot_NeoPixel::showRed(){
    clear();
    setPixelColor(0,Color(this->_brightness,0,0));
    show();
}

void DFRobot_NeoPixel::showGreen(){
    clear();
    setPixelColor(0,Color(0,this->_brightness,0));
    show();
}

void DFRobot_NeoPixel::showBlue(){
    clear();
    setPixelColor(0,Color(0,0,this->_brightness));
    show();
}

void DFRobot_NeoPixel::showYellow(){
    clear();
    setPixelColor(0,Color((this->_brightness)/2,this->_brightness,0));
    show();
}

void DFRobot_NeoPixel::showPurple(){
    clear();
    setPixelColor(0,Color((this->_brightness/2),0,this->_brightness));
    show();
}

void DFRobot_NeoPixel::showCyanblue(){
    clear();
    setPixelColor(0,Color(0,this->_brightness,this->_brightness));
    show();
}

void DFRobot_NeoPixel::showWhite(){
    clear();
    setPixelColor(0,Color((this->_brightness)/2,this->_brightness,this->_brightness));
    show();
}

void DFRobot_NeoPixel::on(){
    clear();
    setPixelColor(0,this->nowColor);
    show();
}

void DFRobot_NeoPixel::FastFlash(){
    clear();
    setPixelColor(0,0);
    show();
    delay(100);
...
void DFRobot_NeoPixel::SlowFlash(){
    delay(1000);
    clear();
    setPixelColor(0,0);
    show();
...
void DFRobot_NeoPixel::BurstFlash(){
    clear();
    setPixelColor(0,0);
    show();
    delay(2900);
...
void DFRobot_NeoPixel::Breathing(uint16_t DTB, uint16_t BTD){
    uint8_t r = 0,g = 0, b = 0;
    clear();
    for(int i = 0; i < (this->_brightness); i+=1){
        if(this->_r > 0){
            r = i/2;
        }
        if(this->_g > 0){
        ...
        show();
        delay(DTB/(this->_brightness));
    }
    for(int i = (this->_brightness); i > 0; i-=1){
        if(this->_r > 0){
            r = i/2;
        }
        if(this->_g > 0){
        ...
        setPixelColor(0, Color(r,g,b));
        show();
        delay(BTD/(this->_brightness));
    }
    
}

void DFRobot_NeoPixel::rainbow(uint8_t wait){
    for(long i = 0; i < 5 * 65536; i += 256){
        int temp = i + 65536L;
        setPixelColor(0,gamma32(ColorHSV(temp)));
        show();
        delay((uint32_t)wait);
    }
    /*
...
void DFRobot_NeoPixel::ConstantlyBright(){
    clear();
    setPixelColor(0,this->nowColor);
    show();
}
